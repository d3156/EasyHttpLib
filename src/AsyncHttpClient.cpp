...
namespace d3156
{
    ...
    net::awaitable<resp_dynamic_body> AsyncHttpClient::sendAsync(req_string_body req, std::chrono::milliseconds timeout)
    {
        static std::atomic<bool> busy_{false};
        size_t spins = 0;
        while (busy_.exchange(true)) {
            if (++spins > 100) co_return resp_dynamic_body{http::status::bad_request, 11};
    ...
    net::awaitable<resp_dynamic_body> AsyncHttpClient::postAsync(std::string path, std::string body,
    ...
    net::awaitable<resp_dynamic_body> AsyncHttpClient::getAsync(std::string path, std::string body,
                                                                std::chrono::milliseconds timeout)
    {
    ...
    {
        if (use_ssl_ && stream_) return stream_->next_layer().socket().is_open();
        return tcp_stream_ && tcp_stream_->socket().is_open();
    }
